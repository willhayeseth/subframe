...
import {
  ...
  Terminal, Globe, Hash, ArrowUpRight, Move
} from "lucide-react";
import {
...
/* ─── subframe network map (pannable) ──────────────────── */

function SubframeNetworkMap({ subdomains, currentName }: { subdomains: Subdomain[]; currentName: string }) {
  ...
  // useLayoutEffect: runs before browser paints — no flash of wrong position
  useLayoutEffect(() => {
    const el = outerRef.current;
    if (!el) return;
